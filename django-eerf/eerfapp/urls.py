...
# calls subject_detail(request=<HttpRequest object>, subject_id='123')
...
urlpatterns = [
    ...
    # /eerfapp/get_xy/ ==> GET data of detail (x) and feature (y) for subject_pk. Uses session for trial_start and
    # trial_stop
    ...
    # Show trial data for a specific subject.
    url(r'^subject/(?P<pk>\d+)/view_data/$', views.view_data, name='view_data'),
    # POST a dict of detail kvps for subject pk
    url(r'^subject/(?P<pk>\d+)/set_details/$', views.set_details, name='set_details'),
    # GET the values for a detail name
    url(r'^subject/(?P<pk>\d+)/detail_values/(?P<detail_name>\w*)/$', views.get_detail_values),
    # GET the values for a feature name
    url(r'^subject/(?P<pk>\d+)/feature_values/(?P<feature_name>\w*)/$', views.get_feature_values),
    # GET the number of trials, filtered by session variables.
    url(r'^subject/(?P<pk>\d+)/count_trials/$', views.count_trials),
    # GET the erp_data for this subject, filtered by session variables.
    url(r'^subject/(?P<pk>\d+)/erp_data/$', views.erp_data),
    # Recalculate feature values
    url(r'^subject/(?P<pk>\d+)/recalculate_feature/(?P<feature_name>\w+)/$', views.recalculate_feature),
    url(r'^period/(?P<pk>\d+)/$',
        DetailView.as_view(
            model=models.Datum,
